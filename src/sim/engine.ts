...
import type { DebrisFragment, SimEvent, SimState, TimelinePoint } from "../types/sim";
...
export function tickSimulation(prev: SimState): Partial<SimState> {
    ...
    for (const band of nextBands) {
        ...
        band.satelliteCount = nextSatellites.filter((s) => s.bandId === band.id &&
            s.active).length;
        band.risk = computeBandRisk(band, prev.config);
        if (shouldTriggerCollision(band.risk)) {
            collisionsThisTick += 1;
            const victims = nextSatellites.filter((s) => s.bandId === band.id &&
                s.active).slice(0, 2);
            victims.forEach((sat) => {
                sat.active = false;
            });
